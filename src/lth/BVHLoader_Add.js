...
/*THREE.BVHLoader.prototype.testType = function ( source ) {

    var list = [];

    for ( var i = 0; i < source.length; i ++ )  list.push( source[ i ].name );

    //console.log( list )

    if( (list.indexOf('LeftShoulder')!==-1) && (list.indexOf('LeftArm')!==-1) ) this.isMixamoType = true;
    else this.isMixamoType = false;
...
THREE.BVHLoader.prototype.applyToModel = function ( model, result, seq, Pos, extraModel ) {
